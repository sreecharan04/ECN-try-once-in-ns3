...
TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (tcpflags == 0)
    ...
    { 
     ...
     // Handshake completed
     // std::cout<<"Receiving SYN/ACK with not-ect mark\n";
      NS_LOG_DEBUG ("SYN_SENT -> ESTABLISHED");
      m_congestionControl->CongestionStateSet (m_tcb, TcpSocketState::CA_OPEN);
      m_state = ESTABLISHED;
     //  std::cout<<"Sender side connection established\n";
     // std::cout<<m_tcb->m_ecnState<<"-sender side ecn state\n";
      m_connected = true;
      m_retxEvent.Cancel ();
      m_rxBuffer->SetNextRxSequence (tcpHeader.GetSequenceNumber () + SequenceNumber32 (1));
      m_tcb->m_highTxMark = ++m_tcb->m_nextTxSequence;
  ...
  if (tcpflags == 0
   ...
   { 
       ...
       if (m_ecnMode == EcnMode_t::ClassicEcn && (tcpHeader.GetFlags () & TcpHeader::ECE))
        {
          ...
          // If the tcp responser is the sender, it will send out data from 1 Segment
          // If the tcp responser is the receiver, we reduce the congestion window but it doesn't affect the congestion in any way.
          m_tcb->m_cWnd = 1;
          m_tcb->m_cWndInfl = m_tcb->m_cWnd;
          m_retxEvent.Cancel();
          m_tcb->m_highTxMark = ++m_tcb->m_nextTxSequence;
       ...
       // handshake is completed nicely.
        ...
        m_congestionControl->CongestionStateSet (m_tcb, TcpSocketState::CA_OPEN);
      //  std::cout<<m_tcb->m_ecnState<<"-receiver side ecn state\n";
        m_state = ESTABLISHED;
        m_connected = true;
      //  std::cout<<"Connection established on responder side\n";
        m_retxEvent.Cancel ();
        m_tcb->m_highTxMark = ++m_tcb->m_nextTxSequence;
        m_txBuffer->SetHeadSequence (m_tcb->m_nextTxSequence);
       if (m_endPoint)
    ...
    { // Other in-sequence input
      if (tcpflags != TcpHeader::RST)
        { // When (1) rx of SYN+ACK; (2) rx of FIN; (3) rx of bad flags
          ...
          std::cout<<"Reset called on receiver's side\n";
          if (m_endPoint)
            {
              m_endPoint->SetPeer (InetSocketAddress::ConvertFrom (fromAddress).GetIpv4 (),
                                   InetSocketAddress::ConvertFrom (fromAddress).GetPort ());
