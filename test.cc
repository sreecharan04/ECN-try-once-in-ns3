...
void InstallPacketSink (Ptr<Node> node, uint16_t port,Time timeToStart)
{
  PacketSinkHelper sink ("ns3::TcpSocketFactory",
                         InetSocketAddress (Ipv4Address::GetAny (), port));
  ApplicationContainer sinkApps = sink.Install (node);
  sinkApps.Start (timeToStart);
  sinkApps.Stop (Seconds (stopTime));
}

void InstallBulkSend (Ptr<Node> node, Ipv4Address address, uint16_t port, 
                      uint32_t nodeId, uint32_t cwndWindow,
                      Callback <void, uint32_t, uint32_t> CwndTrace,Time timeToStart,double stopTime)
{
...
int main (int argc, char *argv[])
{
  ...
  uint16_t port = 50000;
  InstallPacketSink (rightNodes.Get (0), port,Seconds (0.0));      // A Sink 0 Applications
  InstallPacketSink (rightNodes.Get (1), port,Seconds (0.0));      // B Sink 0 Applications  
  InstallPacketSink (rightNodes.Get (2), port,Seconds (0.0));      // C Sink 0 Applications   
  InstallPacketSink (rightNodes.Get (3), port,Seconds (0.0));      // D Sink 0 Applications
  InstallPacketSink (rightNodes.Get (4), port,Seconds (0.0));      // E Sink 0 Applications

  
  InstallBulkSend (leftNodes.Get (0), routerToRightIPAddress [0].GetAddress (1), port, 2, 0, MakeCallback (&CwndChangeA),Seconds (0.0),10.00);
  InstallBulkSend (leftNodes.Get (1), routerToRightIPAddress [1].GetAddress (1), port, 3, 0, MakeCallback (&CwndChangeB),Seconds (0.0),10.00);
  InstallBulkSend (leftNodes.Get (2), routerToRightIPAddress [2].GetAddress (1), port, 4, 0, MakeCallback (&CwndChangeC),Seconds (0.0),10.00);
  InstallBulkSend (leftNodes.Get (3), routerToRightIPAddress [3].GetAddress (1), port, 5, 0, MakeCallback (&CwndChangeD),Seconds (0.0),10.00);
  InstallBulkSend (leftNodes.Get (4), routerToRightIPAddress [4].GetAddress (1), port, 6, 0, MakeCallback (&CwndChangeE),Seconds (0.0),10.00);
